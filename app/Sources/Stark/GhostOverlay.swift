...
final class GhostOverlay {
    ...
    init() {
        if #available(macOS 26.0, *) {
            ...
            g.cornerRadius = 12
            // `.regular` is the system default — what Writing Tools and the
            // volume HUD use. `.clear` looks more like glass in isolation but
            // barely separates from a dark editor, leaving the card and the
        ...
        p.isOpaque = false
        // Glass alone doesn't lift the card off busy content; the shadow is
        // what gives it separation.
        p.hasShadow = true
    ...
    func showCard(_ text: String, caret: CGRect?, field: CGRect?,
                  window: CGRect?, streaming: Bool = false) {
